...
{
    path_abs = ros::package::getPath("lawn_tractor_navigation") + path_local;
...
int main(int argc, char** argv)
{
    ...
    ros::Publisher pubWaypointNodeEnded = n.advertise<std_msgs::Bool>("/status/waypoint_following_status", 100);
    ...
    for(iter = waypointVect.begin(); iter < waypointVect.end(); iter++)
    {
        ...
        ROS_INFO("Received Map X goal:%.8f", map_point);
        ROS_INFO("Received Map Y goal:%.8f", map_next);
